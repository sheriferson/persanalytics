...
I use a [Garmin Edge 500][https://buy.garmin.com/en-US/US/into-sports/cycling/edge-500/prod36728.html] to track all my training and leisurely rides. After uploading the rides to the [Garmin Connect][http://connect.garmin.com/en-US/], I can export a `.csv` file, which is what I use for the plotting below.

```{r importCycling, message=FALSE}
cycling <- read.csv("cycling.csv")
```

```{r cyclingCleanup, message=FALSE}
